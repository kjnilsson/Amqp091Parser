...
type BasicPropsData = 
    { _data: Map<string, Field> }
    member p.ContentType with get() = match Map.tryFind "content-type" p._data with Some (ShortStrField ct) -> ct | _ -> null
    static member parse (payload: byte []) : BasicPropsData =
        let off = 12
        let bit = 0
        ...
        { _data = table }
    static member pickle (props: BasicPropsData) =
        let fields = [
            "content-type"
            "content-encoding"
            "headers"
        ...
        let _, flags, data =
            ...
            |> List.fold (fun (i, s, data) v -> 
                match Map.tryFind v props._data with
...
let test () =
    ...
    let payload = Array.zeroCreate 12 ++ BasicPropsData.pickle { _data = p }
    let off = 12
    let bit = 0
    let off, flags = readShort payload off
