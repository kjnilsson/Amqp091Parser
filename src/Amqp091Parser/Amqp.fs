...
let inline readOctet (data: byte[]) offset =
    offset + 1, data.[offset]

let inline readShort (data: byte[]) offset =
    offset + 2, toShort data offset

let inline readShortInt (data: byte[]) offset =
    offset + 2, toShortInt data offset

let inline readLong (data: byte[]) offset =
    offset + 4, asLong data offset

let inline readLongInt (data: byte[]) offset =
    offset + 4, toLongInt data offset

let inline readLongLong (data: byte[]) offset =
    offset + 8, asLongLong data offset

let inline readLongLongInt (data: byte[]) offset =
    offset + 8, asLongLongInt data offset

let inline readFloat (data: byte[]) offset =
    offset + 8, toFloat data offset

let inline readDouble (data: byte[]) offset =
    offset + 8, toDouble data offset

let inline readDecimal (data: byte[]) offset =
    offset + 5, data.[offset .. offset + 4] //TODO: actually parse this

let inline readLongStr (data: byte[]) offset =
    let len = asLong data offset |> int
    offset + 4 + len, (data.[offset + 4 .. offset + 3 + len])

let inline readChar (data: byte[]) offset =
    offset+1, data.[offset] |> char

let readTimestamp = readLongLong 
    
let inline readShortStr (data: byte[]) offset =
...
let inline writeOctet (o: Octet) = [| o |]
